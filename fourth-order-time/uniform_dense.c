...
{
  float *r6_vec;
  posix_memalign((void**)(&r6_vec),64,(x_size + 2)*(y_size + 2)*sizeof(float));

  float (*restrict damp)[damp_vec->size[1]] __attribute__ ((aligned (64))) = (float (*)[damp_vec->size[1]]) damp_vec->data;
  float (*restrict r6)[y_size + 2] __attribute__ ((aligned (64))) = (float (*)[y_size + 2]) r6_vec;
  ...
  for (int time = time_m, t0 = (time)%(491), t1 = (time + 490)%(491), t2 = (time + 1)%(491); time <= time_M; time += 1, t0 = (time)%(491), t1 = (time + 490)%(491), t2 = (time + 1)%(491))
  {
    ...
    for (int x = x_m - 1; x <= x_M + 1; x += 1)
    {
      ...
      {
        r6[x + 1][y + 1] = (s*s)*((1.0F/12.0F)*(vp[x + 2][y + 2]*vp[x + 2][y + 2])*(4.0e-4F*(u[t0][x + 1][y + 2] + u[t0][x + 2][y + 1] + u[t0][x + 2][y + 3] + u[t0][x + 3][y + 2]) - 1.59999996e-3F*u[t0][x + 2][y + 2])) + u[t0][x + 2][y + 2];
      }
    }
    for (int x = x_m; x <= x_M; x += 1)
    {
      ...
      {
        float r7 = 1.0F/(vp[x + 2][y + 2]*vp[x + 2][y + 2]);
        u[t2][x + 2][y + 2] = (r5*damp[x + 1][y + 1]*u[t0][x + 2][y + 2] + r7*(-r4*(-2.0F*u[t0][x + 2][y + 2]) - r4*u[t1][x + 2][y + 2]) + 4.0e-4F*(r6[x][y + 1] + r6[x + 1][y] + r6[x + 1][y + 2] + r6[x + 2][y + 1]) - 1.59999996e-3F*r6[x + 1][y + 1])/(r4*r7 + r5*damp[x + 1][y + 1]);
      }
    }
    ...
    for (int p_src = p_src_m; p_src <= p_src_M; p_src += 1)
    {
      ...
      {
        float r0 = (s*s)*(vp[ii_src_0 + 2][ii_src_1 + 2]*vp[ii_src_0 + 2][ii_src_1 + 2])*(4.0e-4F*px*py - 2.0e-2F*px - 2.0e-2F*py + 1)*src[time][p_src];
        u[t2][ii_src_0 + 2][ii_src_1 + 2] += r0;
      }
      if (ii_src_0 >= x_m - 1 && ii_src_2 >= y_m - 1 && ii_src_0 <= x_M + 1 && ii_src_2 <= y_M + 1)
      {
        float r1 = (s*s)*(vp[ii_src_0 + 2][ii_src_2 + 2]*vp[ii_src_0 + 2][ii_src_2 + 2])*(-4.0e-4F*px*py + 2.0e-2F*py)*src[time][p_src];
        u[t2][ii_src_0 + 2][ii_src_2 + 2] += r1;
      }
      if (ii_src_1 >= y_m - 1 && ii_src_3 >= x_m - 1 && ii_src_1 <= y_M + 1 && ii_src_3 <= x_M + 1)
      {
        float r2 = (s*s)*(vp[ii_src_3 + 2][ii_src_1 + 2]*vp[ii_src_3 + 2][ii_src_1 + 2])*(-4.0e-4F*px*py + 2.0e-2F*px)*src[time][p_src];
        u[t2][ii_src_3 + 2][ii_src_1 + 2] += r2;
      }
      if (ii_src_2 >= y_m - 1 && ii_src_3 >= x_m - 1 && ii_src_2 <= y_M + 1 && ii_src_3 <= x_M + 1)
      {
        float r3 = 4.0e-4F*px*py*(s*s)*(vp[ii_src_3 + 2][ii_src_2 + 2]*vp[ii_src_3 + 2][ii_src_2 + 2])*src[time][p_src];
